...
class StoreOrder extends ModelBasic
{
    ...
    public static function orderCount()
    {
            ...
            $data['sh'] = self::statusByWhere(2, new self())->count();
            $data['tk'] = self::statusByWhere(-1, new self())->count();
            $data['yt'] = self::statusByWhere(-2, new self())->count();
            $data['sp'] = self::statusByWhere(8, new self())->count();
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            if(!$item['_info']) $item['_info']['title']='不存在';
            if ($item['paid'] == 1) {
...
HTML;
    ...
    public static function SaveExcel($list)
    {
        ...
        PHPExcelService::setExcelHeader(['订单号', '支付方式', '商品总数', '商品总价', '邮费', '支付金额', '退款金额', '商品信息', '推广人', '推广人上级', '支付状态','订单状态', '微信昵称', '手机号'])
    ...
    public static function statusByWhere($status, $model = null, $alert = '')
    {
        ...
        if ('' === $status)
            ...
            return $model->where($alert . 'paid', 0)->where($alert . 'is_del', 0)->where($alert . 'status', 0)->where($alert . 'refund_status', 0)->where($alert . 'type', 'in','0,2');
        else if ($status == 1)//已支付 未发货
            return $model->where($alert . 'paid', 1)->where($alert . 'is_del', 0)->where($alert . 'status', 0)->where($alert . 'refund_status', 0)->where($alert . 'type', 'in','0,2');
        else if ($status == 2)//已支付 已发货
            return $model->where($alert . 'paid', 1)->where($alert . 'is_del', 0)->where($alert . 'status', 1)->where($alert . 'refund_status', 0)->where($alert . 'type', 2);
        else if ($status == 5)//课程订单
            return $model->where($alert . 'combination_id', 0)->where($alert . 'is_del', 0)->where($alert . 'is_gift', 0)->where($alert . 'type', 0);
        else if ($status == 6)// 拼团订单
            return $model->where($alert . 'combination_id','>', 0)->where($alert . 'is_del', 0)->where($alert . 'is_gift', 0)->where($alert . 'type', 0);
        else if ($status == 7)// 礼物订单
            return $model->where($alert . 'combination_id', 0)->where($alert . 'is_del', 0)->where($alert . 'is_gift','>', 0)->where($alert . 'type', 0);
        else if ($status == 8)//商品订单
            return $model->where($alert . 'combination_id', 0)->where($alert . 'is_del', 0)->where($alert . 'is_gift', 0)->where($alert . 'type', 2);
        else if ($status == -1)//退款中
            return $model->where($alert . 'paid', 1)->where($alert . 'is_del', 0)->where($alert . 'refund_status', 1)->where($alert . 'type', 2);
        else if ($status == -2)//已退款
            return $model->where($alert . 'paid', 1)->where($alert . 'is_del', 0)->where($alert . 'refund_status', 2)->where($alert . 'type','in', '0,2');
        else
            return $model;
    }
        ...
        if($where['type'] != ''){
            switch ($where['type']){
                ...
                case 7:
                    ...
                    break;
                case 8:
                    $model = $model->where($aler . 'type', 2)->where($aler . 'combination_id', 0)->where($aler .'is_gift',0);
                    break;
            }
        }else{
            $model = $model->where($aler . 'type', 'in','0,2');
        }
        if ($where['data'] !== '') {
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    {
        return self::where(['uid' => $where['uid']])->where('type','<>',1)
            ->order('add_time desc')
