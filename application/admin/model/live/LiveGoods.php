...
class LiveGoods extends ModelBasic
{
    ...
    public static function getLiveStoreProductList($where)
    {
        ...
        $data = $data->join('StoreProduct s','g.special_id=s.id')->join('StoreCategory c', 'c.id=s.cate_id')->field('g.live_id as live_id, g.id as live_goods_id, g.sort as gsort, g.fake_sales as gfake_sales, g.is_show as gis_show, g.sales as gsales, s.*,c.cate_name');
        $data = $data->page((int)$where['page'],(int)$where['limit'])->select();
        $data = count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            $item['_add_time']  = date('Y-m-d H:i:s',$item['add_time']);
        }
        $count = self::alias('g')->join('StoreProduct s','g.special_id=s.id')->count();
        return compact('data','count');
    }
    ...
    public static function getLiveProductLists($live_id,$type=0)
    {
        ...
        $data = $data->where('g.live_id',$live_id);
        $data = $data->where('g.type',$type);
        $data = $data->join('StoreProduct s','g.special_id=s.id')->field('g.live_id as live_id, g.id as live_goods_id, g.sort as gsort, g.fake_sales as gfake_sales, g.is_show as gis_show, g.sales as gsales, s.*');
        $data = $data->select();
        $data = count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            $item['_add_time']  = date('Y-m-d H:i:s',$item['add_time']);
