...
class AliyunLive extends AuthController
{
    ...
    public function create($id=0)
    {
        ...
        $form = Form::create(Url::build('save',['id'=>$id]), [
            ...
            Form::frameImageOne('live_gift_show_img', '图标', Url::build('admin/widget.images/index', array('fodder' => 'live_gift_show_img')), isset($gift) ? $gift->live_gift_show_img : '')->icon('image')->width('70%')->height('500px'),
        ...
        $form->setMethod('post')->setTitle($id ? '修改礼物':'添加礼物')->setSuccessScript('parent.$(".J_iframe:visible")[0].contentWindow.location.reload();');
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
