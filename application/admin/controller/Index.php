...
class Index extends AuthController
{
    ...
    public function main()
    {
        ...
        $topData['orderDeliveryNum'] = StoreOrderModel::where('status', 0)
            ->where('paid', 1)->where('type', 2)->where('is_del',0)
            ->where('refund_status', 0)
            ->count();
        //退换货订单数
        $topData['orderRefundNum'] = StoreOrderModel::where('paid', 1)->where('type', 2)
            ->where('refund_status', 'IN', '1')->where('is_del',0)
            ->count();
        //库存预警
        ...
        //订单数->今日
        $now_day_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $now_day)->count();
        $pre_day_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $pre_day)->where('pay_time', 'lt', $now_day)->count();
        $first_line['d_num'] = [
        ...
        //交易额->今天
        $now_month_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $now_day)->value('sum(pay_price)');
        $pre_month_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $pre_day)->where('pay_time', 'lt', $now_day)->value('sum(pay_price)');
        $first_line['d_price'] = [
        ...
        //交易额->月
        $now_month_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $now_month)->value('sum(pay_price)');
        $pre_month_order_p = StoreOrderModel::where('paid', 1)->where('is_del',0)->where('pay_time', 'gt', $pre_month)->where('pay_time', 'lt', $now_month)->value('sum(pay_price)');
        $first_line['m_price'] = [
            'data' => $now_month_order_p > 0 ? $now_month_order_p : 0,
            'percent' => abs($now_month_order_p - $pre_month_order_p),
            'is_plus' => $now_month_order_p - $pre_month_order_p > 0 ? 1 : ($now_month_order_p - $pre_month_order_p == 0 ? -1 : 0)
        ...
        //本月订单总数
        $now_order_info_c = StoreOrderModel::where('add_time', 'gt', $now_month)->where('is_del',0)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time', 'gt', $pre_month)->where('is_del',0)->where('add_time', 'lt', $now_month)->count();
        $order_info['first'] = [
        ...
        $now_order_info_c = StoreOrderModel::where('add_time', 'gt', $pre_month)->where('is_del',0)->where('add_time', 'lt', $now_month)->count();
        $pre_order_info_c = StoreOrderModel::where('add_time', 'gt', $second_pre_month)->where('is_del',0)->where('add_time', 'lt', $second_now_month)->count();
        $order_info["second"] = [
            'data' => $now_order_info_c ? $now_order_info_c : 0,
            'percent' => abs($now_order_info_c - $pre_order_info_c),
            'is_plus' => $now_order_info_c - $pre_order_info_c > 0 ? 1 : ($now_order_info_c - $pre_order_info_c == 0 ? -1 : 0)
