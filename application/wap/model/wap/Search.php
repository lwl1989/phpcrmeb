...
class Search extends ModelBasic
{
    ...
    public static function getSearchContent($search, $limit = 3,$uid=0, $page = 0)
    {
        ...
        $exists_search_reids = $redisModel->HEXISTS("wap_index_has","search_history_".$uid);
        if ($exists_search_reids) {
            $data['id'] = $id;
            $search_list_redis = json_decode($redisModel->hget("wap_index_has","search_history_".$uid),true);
            $redis_tmp = array_merge([$data], $search_list_redis);
            $redisModel->hset("wap_index_has","search_history_".$uid, json_encode($redis_tmp));
        }
        return $searchList;
    }
