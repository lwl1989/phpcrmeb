            ...
            if ($special['type'] == SPECIAL_COLUMN) {
                ...
                if(count($specialSource) > 0){
                    foreach ($specialSource as $sk => $sv) {
                        $task = SpecialTask::where('is_show',1)->where('id',$sv['source_id'])->field('id,special_id,title,detail,type,is_pay,link,image,abstract,sort,play_count,is_show,add_time,live_id,videoId,file_name,file_type')->order('sort DESC')->find();
