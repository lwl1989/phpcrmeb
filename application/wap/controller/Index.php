...
class Index extends AuthController
{
    ...
    public function index()
    {
        ...
        //热门直播
        $exists_live_reids = $this->redisModel->HEXISTS("wap_index_has","live_list");
        if (!$exists_live_reids) {
            $liveList = json_encode(LiveStudio::getLiveList(10));
            $this->redisModel->hset("wap_index_has","live_list", $liveList);
            $this->redisModel->expire("wap_index_has",120);
        }else{
            $liveList = $this->redisModel->hget("wap_index_has","live_list");
        }
        $this->assign([
            ...
            'liveList' => $liveList,
            'eventList' => json_encode(EventRegistration::eventRegistrationList(1,1)),
    ...
    {
        //获取推荐列表
        $exists_recommend_reids = $this->redisModel->HEXISTS("wap_index_has","recommend_list");
        if (!$exists_recommend_reids) {
            $recommend_list = json_encode(Recommend::getContentRecommend((int)$page, (int)$limit, $this->uid));
            $this->redisModel->hset("wap_index_has","recommend_list", $recommend_list);
            $this->redisModel->expire("wap_index_has",120);
        }else{
            $recommend_list = $this->redisModel->hget("wap_index_has","recommend_list");
        }
    ...
    public function get_search_history($search = '', $limit = 0)
    {
       ...
       if($uid) {
           $exists_search_reids = $this->redisModel->HEXISTS("wap_index_has","search_history_".$uid);
           if (!$exists_search_reids) {
               $search_list = Search::userSearchHistory($uid) ? json_encode(Search::userSearchHistory($uid)) : [];
               if ($search_list) {
                   $this->redisModel->hset("wap_index_has","search_history_".$uid, $search_list);
                   $this->redisModel->expire("wap_index_has",120);
               }else{
                   $this->redisModel->hdel("wap_index_has","search_history_".$uid);
               }
           }else{
               $search_list = $this->redisModel->hget("wap_index_has","search_history_".$uid);
           }
       }else{
           $search_list=[];
       }
