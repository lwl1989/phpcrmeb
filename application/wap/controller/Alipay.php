...
class Alipay extends WapBasic
{
    ...
    public function alipay_success_synchro()
    {
        ...
        if (isset($res['get']['trade_no']) && isset($res['get']['out_trade_no']) && isset($res['get']['passback_params'])){
            if (($count = strpos($res['get']['out_trade_no'], '_')) !== false) {
                $res['get']['out_trade_no'] = substr($res['get']['out_trade_no'], $count + 1);
            }
            ...
            }elseif ($res['get']['passback_params']=='goods'){
                StoreOrder::where('order_id', $res['get']['out_trade_no'])->where('type',2)->update(['trade_no' => $res['get']['trade_no']]);
            }
        }
