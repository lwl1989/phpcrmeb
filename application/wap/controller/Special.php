...
class Special extends AuthController
{
    ...
    public function task_info($id = 0, $specialId = 0)
    {
        ...
        if ($isPay || $special->pay_type == 0 /*&& !$special->is_pink*/ || ($user_level['level'] > 0 && $special->member_pay_type == 0)) {
            $isPay = true;
        }else{
            $special_source = SpecialSource::where(['special_id' => $specialId, 'pay_status' => 0])->find();
            if (!$special_source) {
                $taskInfo = [];
            }else{
                ...
                if(!$taskInfo){
                    return $this->failed('没有免费素材',Url::build('special/details',array('id'=>$specialId)));
                }
                $taskInfo->content =  $content;
            }
        }
    ...
    public function pay_order($id = 0,$payType)
    {
        ...
        if($payType=='weixin' || $payType=='zhifubao'){
